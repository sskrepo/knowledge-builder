...
class TestRC1ExecutorResolvePinnedRef:
    ...
    def test_pinned_page_found_in_kb_returns_matching_passages(self):
        """When pinned page is in KB (with canonical_ref stamped), executor returns only that page's passages.

        ADR-039: passage matching uses canonical_ref.canonical_id == canonical_id.
        The passage must carry metadata.canonical_ref (stamped by normalize() at INGEST time).
        """
        from framework.workflow_runtime.executor import WorkflowExecutor, ConfluencePageNotInKBError
        pinned_page_id = "12345678901"
        # Use a URL that fast-path extraction resolves to the numeric ID (pageId= form)
        ...
        # ADR-039: passage must carry canonical_ref stamped at INGEST time.
...
# ===========================================================================
# RC1-A Tests — display-by-title URL pinned_ref matching (DECISION-019/ADR-039)
# ===========================================================================
# ADR-039 (DECISION-020): The RC1-A heuristic functions (_is_display_url,
# _extract_display_url_parts, _passage_matches_display_url, _passage_matches_page_id)
# have been DELETED. Display-by-title URL resolution is now handled inside
# resolve_to_numeric_id() in adapters/confluence/shared.py via canonical_identity().
# Tests for those deleted helpers are replaced by tests in test_executor_source_guard.py
# (TestPassageMatchesCanonical) and test_source_identity.py (multi-form canonicalization).
# ===========================================================================
