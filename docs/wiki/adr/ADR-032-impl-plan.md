...
| **P1-E** Re-author four affected TPM skills | `framework/workflow_skills/tpm/project_tracking_confluence_stakeholder_status_meeting_email.yaml` + three sibling files: add `source_binding` block + change `trigger.on_request.inputs[0]` from `{name:input, type:string}` to `{name:page_id, type:confluence_page_ref, required:true}` | YES (YAML files; no .py) | P1-B complete (so DESIGN_SKILL knows to emit ask_parameterized) | No new tests; manual smoke: `kb-cli workflow-run tpm.project_tracking_confluence_stakeholder_status_meeting_email --inputs '{"page_id":"18625350641"}'` triggers ephemeral fetch (or hard-fail with adapter-missing message in laptop mode without Keychain) | Each of the four skill YAMLs has `source_binding.mode: ask_parameterized`, `input_param: page_id`, `ingest_on_demand: true`, `space_allow_list: [FA, PROJ]`; old `{name:input, type:string}` input removed |
