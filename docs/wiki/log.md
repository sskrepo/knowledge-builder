...
## [2026-05-18] backend-dev | feat(adr-036): connector discoverability — listConnectors MCP tool + proactive CONFIGURE_SOURCES list

Added two discoverability surfaces to close the gap where users could only learn supported connectors reactively (by triggering the hard-stop). (1) `listConnectors` MCP tool: read-only, no scope required, registered in `EXTERNAL_TOOLS_SCHEMA` and `build_external_tool_registry`, returns 6 user-facing fields per connector (strips `access_probe_hook`/`granularity_filters`), exactly 3 connectors (UDAP absent). (2) Proactive supported-connector block injected into both CONFIGURE_SOURCES entry paths (`_advance_to_configure_sources` + `_advance_to_configure_sources_v2` no-sources path). Both surfaces share one `format_supported_connectors_block()` helper and `manifest_to_user_facing()` projection in `registry.py` — drift-prevention guaranteed. 33 new tests in `test_adr036_discoverability.py` (all pass); `test_kbf_ops_review::test_tools_list_has_expected_tools` updated (8→9 tools). Full suite: 8 pre-existing failures, 0 new. BUG-queue-19f2f filed (discovered_by=architect, severity=MEDIUM, status=fixed) + ADB COUNT=1 verified; export-bugs regenerated. ADR-036 Section P added. Branch only — not merged; main untouched.
...
## [2026-05-18] backend-dev | DECISION-020 / ADR-039 merged to main (user-approved); ADR-039 → Accepted.

## [2026-05-19] backend-dev | DECISION-013 bug fixes committed: BUG-012 (executor Strategy 1b ADB content field never read — fixed in 09aa33c), BUG-013 (Path B routing self-test draft card not injected — fixed in 09aa33c), BUG-014 (routing self-test case-sensitive skill_name fails on LLM caps drift — fixed in 38d4b71). All 3 bugs filed in ADB KBF_BUG_REPORTS (BUG-queue-decision013) and pmo/bugs/. Unit suite: 1792 passed, 8 pre-existing failures, 0 new. SKILL 1 (tpm.faaas_kiwi_project_pptx) authorSkill session synth-tpm-628a9647: VALIDATE passed (after ADB workflow_skill patch to remove erroneous tpm.tpm_weekly_ops requires_extractions entry), INGEST complete (2 pages unchanged), EVAL Path A SUCCESS. EVAL Path B blocked on OCI token expiry (awaiting browser re-auth). Session at EVAL state, server on SHA 38d4b71-dirty (01c3f6b on main). SKILL 2 (query-time TPM email) not started.

## [2026-05-19] backend-dev | BUG-016 fixed: Strategy 1a retriever signature mismatch. `_retrieve_author_fixed_pinned` invoked retrievers with a hard-coded `query=/persona=` shape (never threading `corpus` for `vector_search`) and the fallback call sat outside `except TypeError`, so a signature mismatch crashed the whole tier-1 render before Strategy 1b could recover. Surfaced live on `faaas_kiwi_project_pptx` (`__call__() missing 1 required positional argument: 'corpus'`). Fix: new signature-introspecting dispatch shim `_invoke_retriever_compat` (handles functions/lambdas/callable-instances, passes only declared params from a {query,nl_query,corpus,persona} pool, skips-not-crashes on unsuppliable required params or any exception → Strategy 1b safety net always reachable). Verified: shim unit cases all pass; full suite 1792 passed / 8 pre-existing baseline failures (identical on stashed HEAD) → 0 new regressions. Bug record: pmo/bugs/BUG-016-strategy1a-retriever-signature-mismatch.md (BUG-queue-decision013). Also RCA'd (surface-only, no fix) askKnowledgeBase id 247: malformed `ttps://` URL → opaque FileNotFoundError; four-layer input-validation/canonicalization gap in ask_parameterized path documented for user decision.

## [2026-05-19] backend-dev | BUG-017 fixed: artifact_url skill returned a synthesizer REFUSAL next to a valid artifact_path (internally contradictory response). Surfaced live on faaas_kiwi_project_pptx (askKB id 251, immediately after BUG-016 unblocked the render path): real 30KB pptx + page cited @1.0, but answer.Answer = "I cannot generate or provide a PPTX…". RCA: tier-1 text synthesizer (runs before maybe_render_artifact) emits a confident refusal to "Generate a PPTX"; the backfill guard only caught empty/sentinel answers, not a verbose refusal → misleading text shipped beside a delivered artifact. Fix (ask.py maybe_render_artifact): added refusal-class detector (`i cannot`/`unable to generate`/…within first 200 chars) gated on `_artifact_delivered`; genuine synthesized summaries still preserved. New regression test test_synthesizer_refusal_with_artifact_is_backfilled; test_real_upstream_answer_is_preserved stays green. Full unit suite: 1793 passed / 8 pre-existing baseline failures (identical on stashed HEAD) → 0 new regressions. Bug record: pmo/bugs/BUG-017-artifact-answer-contradiction-synthesizer-refusal.md (BUG-queue-decision013). Note: the "• (none)" bullet in the same artifact is parked DECISION-023 (extraction fidelity, on hold), not this bug; cost_tokens all-zero on render path tracked separately (telemetry not threaded).
