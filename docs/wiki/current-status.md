...
## Where we are
**Phase 1-3 + V3 deployment layer + laptop mode code complete. ADR-028 S1-S6 live. ADR-029 Phases 1+2 complete. ADR-030 P2 complete. ADR-035 access-gate + single-truth artifact binding (DECISION-015) landed. ADR-036 connector registry + CONFIGURE_SOURCES gating + listConnectors MCP tool merged. ADR-038 + DECISION-018 (consumer-facing card + routing self-test + negative signal fix) landed. DECISION-019 RC1+RC2+Finding-B+RC1-A fixed. ADR-039 (DECISION-020) source-identity canonicalization + DECISION-020 bind-side canonicalization gap fully closed on main (294439c). DECISION-021 (2026-05-18): EVAL Path-B routing self-test now uses production IntentClassifier internally (ADR-038 §B.5 amendment); token-overlap routing-precision loop exited structurally.** 160+ Python files, ~18K+ LOC, 1750+ tests passing (8 pre-existing failures).

**TWO-SKILL E2E MISSION COMPLETE (2026-05-19):** Both `tpm.faaas_kiwi_project_pptx` (author_fixed, PPTX, synth-tpm-628a9647) and `tpm.confluence_project_tracking_rods_support_for_dynamic_tables` (ask_parameterized, .eml email draft, synth-tpm-4bebf913) PROMOTED to production. ADB-backed proof: all 10 artifacts `status=promoted` in `KB_SHIM.KBF_SKILL_ARTIFACTS`. Real artifacts on disk: `~/.kbf/outputs/faaas_kiwi_project_pptx.pptx` (30,057 bytes OOXML) + `~/.kbf/outputs/confluence_project_tracking_rods_support_for_dynamic_tables.email` (17,849 bytes HTML+RFC-822 draft, no SMTP/send path). Path-B routing: 8/8 and 10/10 respectively. Path-A execution: both SUCCESS. BUG-015 (f249c25) fixed: executor `delivery` sub-dict extraction in `_run_eval`. Git SHA at completion: f249c25.

ADR-039 implementation (2026-05-18, branch feat/decision-020-source-identity): 7-iteration RC1/RC1-A regex whack-a-mole cycle ended architecturally. `CanonicalRef`/`Unresolvable`/`AdapterWithIdentity` ABC added. `resolve_to_numeric_id()` 3-step algorithm (fast-path numeric + display-URL title lookup + optional validation). All Confluence adapters + Jira implement `canonical_identity()`. Registry `canonical_identity()` chokepoint added (registry.py IndentationError also fixed). `_retrieve_author_fixed_pinned` rewritten to canonical==canonical. All 7 RC1/RC1-A heuristic functions deleted. `ask.py` D1 extraction updated. Zero new test failures.
