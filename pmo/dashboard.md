...
| [BUG-005](bugs/BUG-005-workflow-kb-name-mismatch.md) | `authorSkill` validation fails — `synthesize_workflow` writes `{persona}.{skill}_data` as KB ref; persona builder registers `{persona}.{skill}` (no suffix); validator can't resolve the reference | blocker | verified | (this commit) |
