...
| [BUG-004](bugs/BUG-004-synthesized-artifacts-not-persisted.md) | `authorSkill` commits 0 artifacts — `synthesized_artifacts` omitted from `to_dict()` serialization; empty dict on session resume → `_write_artifacts()` loops over nothing | blocker | verified | (this commit) |
